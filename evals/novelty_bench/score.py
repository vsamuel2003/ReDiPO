...
async def process_instances(instances, output_file, patience):
    """Processes all instances and writes results to a file, resuming from partial output."""
    done_ids: set = set()
    ...
    if done_ids:
        print(f"Resuming: {len(done_ids)} already scored, {len(pending)} remaining.")
    ...
    async with aio_open(output_file, "a", buffering=1) as f:
        ...
        for result in tqdm(await asyncio.gather(*tasks), total=len(pending)):
...
async def main():
    ...
    # Release the Skywork reward model from VRAM before the next pipeline stage.
    import gc
    rm, _ = rm_and_tokenizer()
    del rm
    rm_and_tokenizer.cache_clear()
    gc.collect()
