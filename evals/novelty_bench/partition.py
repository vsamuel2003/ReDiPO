"""
Partition novelty-bench generations into equivalence classes.
Adapted from novelty-bench/src/partition.py — removed src.common dependency.
"""
...
rouge_scorer = rouge_scorer_module.RougeScorer(["rouge1"])
...
async def classifier_score(prompt: str, s1: str, s2: str):
    ...
    for s in [s1, s2]:
        input_ids.extend(
            tokenizer.encode(s, truncation=True, max_length=128, add_special_tokens=False)
        )
...
async def equivalence_check_classifier(prompt: str, response_0: str, response_1: str) -> bool:
...
async def equivalence_check_unigram(prompt: str, response_0: str, response_1: str) -> bool:
    return await rouge1(prompt, response_0, response_1) > 0.458


async def equivalence_check_bertscore(prompt: str, response_0: str, response_1: str) -> bool:
...
async def partition_responses(prompt: str, responses: list[str], equivalence_alg) -> list[int]:
    equivalence_classes = []
    ...
    for i in range(len(responses)):
        ...
        for j in range(i + 1, len(responses)):
            if partition[j] == -1 and await equivalence_alg(prompt, current_class[0], responses[j]):
...
async def process_instances(instances, output_file, equivalence_alg):
    ...
    if os.path.exists(output_file):
        try:
            ...
            done_ids = set(existing_output["id"])
            if not set(instances["id"]) - done_ids:
                print("All prompts have been partitioned. Skipping.")
                return
        except datasets.exceptions.DatasetGenerationError:
            pass

    pending = [inst for inst in instances if inst["id"] not in done_ids]
    if done_ids:
        print(f"Resuming: {len(done_ids)} already done, {len(pending)} remaining.")
    ...
    async with aio_open(output_file, "a", buffering=1) as f:
        ...
        tasks = [process_single_instance(instance) for instance in pending]
        for task in tqdm(asyncio.as_completed(tasks), total=len(pending)):
            result = await task
...
async def main():
    ...
    parser.add_argument("--alg", default="classifier", choices=EQUIVALENCE_ALGS)
    parser.add_argument("--eval-dir", required=True)
    args = parser.parse_args()

    instances = load_dataset(
        "json",
        data_files=os.path.join(args.eval_dir, "generations.jsonl"),
        split="train",
    )
    output_file = os.path.join(args.eval_dir, "partitions.jsonl")
    await process_instances(instances, output_file, EQUIVALENCE_ALGS[args.alg])

    # Release the DeBERTa classifier from VRAM before the next pipeline stage.
    import gc
    load_deberta_tokenizer_and_model.cache_clear()
    gc.collect()
